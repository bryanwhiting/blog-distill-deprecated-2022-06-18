...
```{r, message=F, warning=F}
plot_ly(iris, x = ~Sepal.Width, y = ~Sepal.Length,type = 'scatter', mode = 'markers', color = ~Species) %>% layout(legend = list(orientation = 'h'))
```
...
```{r, message=F, warning=F}
p = plot_ly(iris, x = ~Sepal.Width, y = ~Sepal.Length,type = 'scatter', mode = 'markers', color = ~Species) 
# Put legend on bottom, change the x-axis range, and turn on the x-axis line. Also, make the zeroline visible, and turn it red.
p = p %>% layout(legend = list(orientation = 'h'),
             xaxis = list(zeroline = T, # Turns x = 0 on
