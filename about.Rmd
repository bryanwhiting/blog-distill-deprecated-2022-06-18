...
tribble(
  ~date, ~company, ~url, ~title, ~location, ~notes,
  # Google
  "2020/07 - Present",
  "Google/YouTube",
  ...
  cols_merge(
    ...
    pattern = (paste0(
      "<a href='{2}' style='float:left;text-decoration:none;'><strong style='color:#007bff'>{1}</strong></a><span style='float:right;'>", pin, " {3}</span> <br>{4}<br><br><span style='color:gray;font-size=.8em'>{5}</span>"
    ))
  ) %>%
  tab_style(style = "vertical-align:top", locations = cells_body(columns = date)) %>%
  tab_options(
